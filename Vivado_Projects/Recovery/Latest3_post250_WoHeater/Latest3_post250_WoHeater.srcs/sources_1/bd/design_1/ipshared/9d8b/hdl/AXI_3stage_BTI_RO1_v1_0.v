...
`timescale 1 ns / 1 ps
	...
	module AXI_3stage_BTI_Pavi_RO1_v1_0 #
	(
...
// Instantiation of Axi Bus Interface S00_AXI
	...
	AXI_3stage_BTI_Pavi_RO1_v1_0_S00_AXI # ( 
		.C_S_AXI_DATA_WIDTH(C_S00_AXI_DATA_WIDTH),
		.C_S_AXI_ADDR_WIDTH(C_S00_AXI_ADDR_WIDTH)
	) AXI_3stage_BTI_Pavi_RO1_v1_0_S00_AXI_inst (
	    .En(En),
	    //.En2(En2),
	...
	// Add user logic here
	(*DONT_TOUCH= "true"*) wire [num_oscillators*5-1 : 0] w;
    ...
    begin : CRO
        //if (i<(num_oscillators/2)) begin
            LUT6_AND AND(                       //AND Gate
                //.control(slv_reg_out[i]),
                .control(En),                   //External stress signal1    
                .in_sig(w[i*4+3]),
                .out_sig(w[i*4])
                );
            LUT6_NOTv2 Inverter0(               //3 inverter chain below
                .in_sig(w[i*4]),
                .out_sig(w[i*4+1])
                );
       ...
       /*end else begin
            LUT6_AND AND(                       //AND Gate
                //.control(slv_reg_out[i]),
                .control(En2),                   //External stress signal2    
                .in_sig(w[i*4+3]),
                .out_sig(w[i*4])
                );
            LUT6_NOTv2 Inverter0(               //3 inverter chain below
       ...
       */ 
    //(*DONT_TOUCH= "true"*) assign w[i*4] = w[i*4+4]; //nand Control(w[0], w[SIZE-1], rst);
    assign input_signal[i] = w[i*4+3];
    /*
    always @(*)
    begin
      input_signal[i] <= w[i*4+3];
    end

        frequency_counter #
    ( .num_counters(i)) frequency_counter_instance
    (
    .in_signal(w[i*4+3]),
    .clk(s00_axi_aclk),
    .freq(frequency_counter_wire[i])
        );
        */
    end
    endgenerate
